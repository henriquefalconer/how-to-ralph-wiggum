...
export async function PATCH(
  request: Request,
  { params }: { params: Promise<{ phaseId: string; fieldId: string }> },
) {
  const { phaseId, fieldId } = await params;
  ...
  try {
    const field = await updateField("phase", phaseId, fieldId, {
...
export async function DELETE(
  ...
  try {
    await deleteField("phase", phaseId, fieldId);
