...
# Run with: npx bats ralph/ralph-watchdog.test.bats

setup() {
  REPO="$BATS_TEST_TMPDIR/repo"
  mkdir -p "$REPO/ralph/.state" "$REPO/bin"
  cp "$BATS_TEST_DIRNAME/ralph-watchdog.sh" "$BATS_TEST_DIRNAME/ralph-resume.sh" "$REPO/ralph/"

  export INSPECT_ARGS="$REPO/inspect-args.txt"
  export BUILD_ARGS="$REPO/build-args.txt"
  export QA_ARGS="$REPO/qa-args.txt"
  export STATE="$REPO/ralph/.state"
  ...
  cat > "$REPO/ralph/ralph-inspect.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$INSPECT_ARGS"
...
STUB
  ...
  cat > "$REPO/ralph/ralph-build.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$BUILD_ARGS"
...
STUB
  ...
  cat > "$REPO/ralph/ralph-qa.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$QA_ARGS"
...
STUB
  ...
  chmod +x "$REPO/ralph"/ralph-{inspect,build,qa}.sh
...
@test "requires a target url" {
  run "$REPO/ralph/ralph-watchdog.sh"
  [ "$status" -ne 0 ]
  [[ "$output" == *"Usage:"* ]]
}
...
@test "refuses to start when a live watchdog already holds the lock" {
  ...
  run "$REPO/ralph/ralph-watchdog.sh" https://example.com
  [ "$status" -eq 0 ]
...
@test "forwards the iteration budgets to each phase" {
  run "$REPO/ralph/ralph-watchdog.sh" https://example.com 3 4 5
  [ "$status" -eq 0 ]
...
exit 0
STUB
  chmod +x "$REPO/ralph/ralph-build.sh"
...
exit 0
STUB
  chmod +x "$REPO/ralph/ralph-qa.sh"
