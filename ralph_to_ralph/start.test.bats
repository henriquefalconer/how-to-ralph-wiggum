...
# Run with: npx bats ralph_to_ralph/start.test.bats

setup() {
  REPO="$BATS_TEST_TMPDIR/repo"
  mkdir -p "$REPO/ralph_to_ralph/.state" "$REPO/bin"
  cp "$BATS_TEST_DIRNAME/start.sh" "$REPO/ralph_to_ralph/"

  export WATCHDOG_ARGS="$REPO/watchdog-args.txt"
  cat > "$REPO/ralph_to_ralph/ralph-watchdog.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$WATCHDOG_ARGS"
exit 0
STUB
  chmod +x "$REPO/ralph_to_ralph/ralph-watchdog.sh"

  printf '#!/bin/bash\nexit 0\n' > "$REPO/bin/sleep"
  chmod +x "$REPO/bin/sleep"
  PATH="$REPO/bin:$PATH"

  LOCKFILE="$REPO/ralph_to_ralph/.state/watchdog.lock"
}

@test "requires a target url" {
  run "$REPO/ralph_to_ralph/start.sh"
  [ "$status" -ne 0 ]
  [[ "$output" == *"Usage:"* ]]
}

@test "creates the per-phase state namespaces before starting" {
  run "$REPO/ralph_to_ralph/start.sh" https://example.com
  [ "$status" -eq 0 ]
