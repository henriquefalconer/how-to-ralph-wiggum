...
# Run with: npx bats ralph_to_ralph/ralph-watchdog.test.bats

setup() {
  REPO="$BATS_TEST_TMPDIR/repo"
  mkdir -p "$REPO/ralph_to_ralph/.state" "$REPO/bin"
  cp "$BATS_TEST_DIRNAME/ralph-watchdog.sh" "$REPO/ralph_to_ralph/"

  export INSPECT_ARGS="$REPO/inspect-args.txt"
  export BUILD_ARGS="$REPO/build-args.txt"
  export QA_ARGS="$REPO/qa-args.txt"
  export STATE="$REPO/ralph_to_ralph/.state"

  # Inspect: records args; writes the completion sentinel unless told not to.
  cat > "$REPO/ralph_to_ralph/ralph-inspect.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$INSPECT_ARGS"
...
STUB
  ...
  cat > "$REPO/ralph_to_ralph/ralph-build.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$BUILD_ARGS"
...
STUB
  ...
  cat > "$REPO/ralph_to_ralph/ralph-qa.sh" <<'STUB'
#!/bin/bash
printf '%s\n' "$@" >> "$QA_ARGS"
exit 0
STUB

  chmod +x "$REPO/ralph_to_ralph"/ralph-{inspect,build,qa}.sh
...
@test "requires a target url" {
  run "$REPO/ralph_to_ralph/ralph-watchdog.sh"
  [ "$status" -ne 0 ]
  [[ "$output" == *"Usage:"* ]]
}
...
@test "refuses to start when a live watchdog already holds the lock" {
  ...
  run "$REPO/ralph_to_ralph/ralph-watchdog.sh" https://example.com
  [ "$status" -eq 0 ]
